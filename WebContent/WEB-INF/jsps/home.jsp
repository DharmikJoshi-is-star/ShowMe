...
.statusstatusstatusstatusClose:hover,
.statusstatusstatusstatusClose:focus {
  color: #f44336;
  cursor: pointer;
}
...
<div id="myNav" class="overlay">
  ...
  <div class="overlay-content">
    <a href="show-group-form?user_id=<c:out value="${user_id}"/>">New group</a>
	...
	<div class="row m-4 border">
		<div class="col-4 border-right pl-0 pr-0"  >
			<nav class="navbar navbar-light my-bg justify-content-center">
				<span class="navbar-brand text-white" style="font-size:20px; cursor:pointer; text-align: left; " onclick="openNav()">&#9776;</span>
				
				<span class="navbar-brand text-white">WhatsApp</span>
			...
			<ul class="list-group list-group-flush" >
				<li class="list-group-item d-flex justify-content-left align-items-center border-bottom p-2">
					<a href="findFriends?userId=<c:out value="${user_id}"/>">
						&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-user-plus" style="font-size:24px;color:light green"></i>
						&nbsp;&nbsp;FIND FRIENDS
					</a>
				</li>
				...
				<c:forEach items="${users}" var="user" >
					<c:if test="${not empty user.getView()}">
						<li class="list-group-item d-flex justify-content-left align-items-center border-bottom p-2">
						<a href="showChat?contact_id=<c:out value="${user.getId()}"/>&user_id=<c:out value="${user_id }"/>">
							<img src="data:image/jpg;base64,${user.getPicture_str()}" alt="" class="rounded-circle my-img mr-2 ml-2" >
							<c:out value="${user.getName() }"/>
							
								<span class="badge badge-primary badge-pill my-pill ml-auto p-1">new</span>
				...
				<c:forEach items="${groups }" var="group">
					<c:if test="${not empty group.getView()}">
						<li class="list-group-item d-flex justify-content-left align-items-center border-bottom p-2">
						<a href="showGroup?group_id=${group.group_id }&user_id=<c:out value="${user_id}"/>">
	...
	<c:choose>
	        ...
	        <div class="col-8">
				...
				<nav class="navbar navbar-light my-bg justify-content-left align-items-center align-middle">
					...
					<span>
						...
						&nbsp;&nbsp;<c:out value="${group.getGroup_name()}"/>
						<span class="navbar-brand text-white"><c:out value="${groupobj.group_name }"></c:out></span>
					</span>
					...
					<div style= "text-align: left;">
						...
						<a href="#">
							<abbr class="fa fa-paperclip" style="font-size:24px;color:black"></abbr>
							
							<input id="choose" type="file" name="sendPicture" style="display: none;">
							<script>
						  ...
						  <i class="fa fa-ellipsis-v" style="font-size:24px;color:black"></i></button>
				...
				<c:forEach items="${groupobj.getMessages()}" var="message" >
			          ...
			          <c:if test="${message.getSender_id() ne user_id }">
			           	...
			           	<p>
			 				...
			 				<span class="time-right" style="font-size: 10px;"><c:out value="${message.getTime()}"/></span>
						</div>
					 </c:if>
					 ...
					 <c:if test="${message.getSender_id() eq user_id }">
			           	...
			           	<p>	
			 				...
			 				<span class="time-left" style="font-size: 10px;"><c:out value="${message.getTime()}"/></span>
			 				</p>
						</div>
					 </c:if>
				...
				<sf:form modelAttribute="gmessage" id="msgbox" action="sentMessageGroup?user_id=${user_id }&group_id=${groupobj.group_id}&tableName=${groupobj.group_msg_table}" method="post">
					<div class="form-row">
						<div class="col mb-2">
							<div class="input-group">
								...
								<sf:input type="text" path="msg" name="message" class="form-control" id="validationTooltipUsername" placeholder="Type your message" aria-describedby="validationTooltipUsernamePrepend" />
								<div class="input-group-prepend  my-bg">
									<button type="submit" class="my-bg"><span class="input-group-text fa my-bg text-white" id="validationTooltipUsernamePrepend">&#xf1d9;</span></button>
								</div>
							</div>
						</div>
					</div>
	    ...
	    <c:when test="${ not empty receiver}">
		    ...
		    <div class="col-8">
				...
				<nav class="navbar navbar-light my-bg justify-content-left align-items-center align-middle">
					<span>
						<img id="myImg" src="data:image/jpg;base64,${receiver.getPicture_str() }" alt="" class="rounded-circle my-img my-bg" >
						<span class="navbar-brand text-white"><c:out value="${receiver.getName() }"></c:out></span>
					</span> 
				...
				<form id="msgbox" action="send-message?user_id=${user_id }&contact_id=${receiver.getId()}" method="post"  >
					<div class="form-row">
						<div class="col mb-2">
							<div class="input-group">
								...
								<input type="text" name="message" class="form-control" id="validationTooltipUsername" placeholder="Type your message" aria-describedby="validationTooltipUsernamePrepend" >
...
<div id="statusId" class="statusModel">
  ...
  <sf:form action="addStatus?user_id=${user_id }" method="post" modelAttribute="userStatus">
  ...
  <div class="container">
      ...
      <hr>
		...
		<abbr  class="fa fa-plus-circle" style="font-size:30px;color:#14FF7A; cursor: pointer;"><span>My status</span></abbr><p>Click to add status update</p>
		<sf:input id="choose" type="file"  path="statusPicture" name="sendPicture" style="display: none;"/>
		<script>
			  $("abbr").on("click", function() {
			    $("input").trigger("click");
			  });
     ...
     <!-- -------------------------------------------------------- -->
      ...
      <div class="row">
	      <c:forEach items="${admin.getStatus() }" var="adminStatus"> 
				  <div class="column">
				    <img src="data:image/jpg;base64,${adminStatus.getStatusPicture() }" 
				    onclick="openModalForAdmin();currentSlideForAdmin(${adminCount})" class="hover-shadow cursor"
		  ...
		  <div class="adminModal-content">
			...
			<c:forEach items="${admin.getStatus() }" var="adminStatus"> 
			    <div class="mySlidesForAdmin">
			      <div class="numbertext"><c:out value="${adminCount }"/> / <c:out value="${totalAdminCount }"/></div>
			      <img src="data:image/jpg;base64,${adminStatus.getStatusPicture() }" style="width:50%; display: block;margin-left: auto;margin-right: auto;">
			      <c:set var="adminCount" value="${adminCount+1 }"/>	
			    </div>
		    </c:forEach>
		
		    <a class="prev" onclick="plusSlidesForAdmin(-1)">&#10094;</a>
		    <a class="next" onclick="plusSlidesForAdmin(1)">&#10095;</a>
			...
			<c:forEach items="${admin.getStatus() }" var="adminStatus"> 
			    <div class="column">
			      ...
			      src="data:image/jpg;base64,${adminStatus.getStatusPicture() }" style="width:100%" 
      	...
      	<c:if test="${not empty user.getStatus() }">
	     	 <div class="row">
			      	<c:forEach items="${user.getStatus() }" var="status">
			      		<div class="column">
				      		<img id="userStatus" src="data:image/jpg;base64,${status.getStatusPicture() }"  onclick="openModal();currentSlide(${count})" class="hover-shadow cursor" alt="status" style="width:10%; border: 1px solid #ddd; border-radius: 50%; padding: 5px; width: 80px; box-shadow: 0 0 2px 1px rgba(0, 140, 186, 0.5);">
 		 ...
 		 <div class="mymodalStatus-content">
	 		 ...
	 		 <c:forEach items="${ users}" var="user">
	      		<c:if test="${not empty user.getStatus() }">
	      			<c:forEach items="${user.getStatus() }" var="status">
				 		 <div class="mySlides">
						      <div class="numbertext"><c:out value="${count }"/> / <c:out value="${totalStatusByFriends }"/> </div>
						      <img src="data:image/jpg;base64,${status.getStatusPicture() }" style="width:50%; display: block;margin-left: auto;margin-right: auto;">
					     </div>
					     <c:set var="count" value="${count+1 }"/>
	 		 		</c:forEach>
	 			 </c:if>
 		  ...
 		  <c:forEach items="${ users}" var="user">
      		<c:if test="${not empty user.getStatus() }">
      			<c:forEach items="${user.getStatus() }" var="status">
			 		 <div class="column">
		      			<img class="demo cursor" src="data:image/jpg;base64,${status.getStatusPicture() }" style="width:100%" 
		      			onclick="currentSlide(${count})" alt="Status By: ${user.getName() }">
		   			 </div>
		   			 <c:set var="count" value="${count+1 }"/>
	 		     </c:forEach>
	 		</c:if>
